...
""")
        ...
        user_prompt_parts.append(f"""
--- 포트폴리오 텍스트 분석 ---
다음 텍스트 내용을 바탕으로 포트폴리오를 분석하고 **객관적으로 요약**해 주세요. 그리고 그 요약 내용을 기반으로 **개선점 및 피드백**을 제공해주세요.
텍스트:
{extracted_text}

[분석 요청]
- **요약과 모든 피드백은 반드시 한국어로 작성해주세요.**
- 핵심 경력, 프로젝트, 성과를 중심으로 객관적으로 요약하여 `summary` 필드에 작성해주세요.
- 기술 스택과 기여한 바를 명확히 해주세요.
- 해당 직무와의 연관성을 고려해주세요.
- 요약 내용을 바탕으로 합격 가능성을 높일 수 있는 구체적인 개선 피드백을 `overall_feedback` 필드에 담아주세요.
...
""")
        ...
        user_prompt_parts.append(f"""
--- 포트폴리오 URL 분석 ---
다음 URL의 포트폴리오를 분석하고 **객관적으로 요약**해 주세요. 그리고 그 요약 내용을 기반으로 **개선점 및 피드백**을 제공해주세요.
URL: {portfolio_url}
...
- 해당 직무와의 연관성을 고려해주세요.
- 요약 내용을 바탕으로 합격 가능성을 높일 수 있는 구체적인 개선 피드백을 `overall_feedback` 필드에 담아주세요.
- 'individual_feedbacks'는 비어있는 객체 {{}}로 반환해주세요. (AI는 실제 URL에 접근할 수 없으므로, 일반적인 포트폴리오 분석 기준에 따라 요약 및 피드백을 제공합니다.)
""")
