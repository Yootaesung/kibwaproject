...
def get_document_analysis_prompt(
    ...
    previous_document_data: Optional[Dict[str, Any]] = None, # 이제 전체 딕셔너리 받음
    older_document_data: Optional[Dict[str, Any]] = None,      # 이제 전체 딕셔너리 받음
    additional_user_context: Optional[str] = None # 사용자가 이전 피드백을 어떻게 반영했는지 설명 (새로 추가)
) -> Tuple[str, str]:
    """
...
"""
    ...
    def format_doc_content(content: Dict[str, Any], d_type: str) -> str:
        # 이력서, 자소서, 포트폴리오 등 문서 타입에 맞춰 내용을 포맷합니다.
        # 이 함수는 content 딕셔너리가 doc_type에 따라 구조화되어 있음을 가정합니다.
        if d_type == "cover_letter":
            ...
            for field_name, label in resume_fields_map.items():
                content_data = content.get(field_name)
                display_content = ""
                if isinstance(content_data, list):
                    # 리스트인 경우 각 항목을 문자열로 변환하여 출력 (예: 학력, 경력)
                    display_content = "\n".join([json.dumps(item, ensure_ascii=False) for item in content_data])
                elif content_data is not None and str(content_data).strip():
                    display_content = str(content_data).strip()
                else:
                    display_content = "작성되지 않음 (누락)"
                formatted_parts.append(f"{label} ({field_name}):\n{display_content}")
            return "\n".join(formatted_parts)
        elif d_type in ["portfolio_summary_url", "portfolio_summary_text"]: # 포트폴리오 요약 요청 시 사용
            if "portfolio_url" in content:
                return f"포트폴리오 URL: {content['portfolio_url']}"
            elif "extracted_text" in content:
                # 너무 긴 텍스트는 잘라서 프롬프트 길이를 조절할 수 있습니다.
                return f"추출된 텍스트:\n{content['extracted_text'][:2000]}..." 
            return str(content) # 그 외의 경우
        elif d_type == "portfolio": # 저장된 포트폴리오 자체를 분석할 때 (main.py의 analyze_document_endpoint에서 호출)
            # 저장된 포트폴리오 데이터는 'content' 필드 아래에 'summary', 'portfolio_link', 'portfolio_pdf_filename' 등을 가짐
    ...
    # 임베딩 기반 검색으로 얻은 관련성 높은 이전 기록을 프롬프트에 포함
...
""")
    elif doc_type == "portfolio": # main.py의 analyze_document_endpoint에서 호출될 때 사용
        user_prompt_parts.append(f"""
--- 포트폴리오 정보 분석 ---
...
[피드백 요청]
- {job_title} 직무 관점에서 포트폴리오의 내용(링크, PDF 파일명, 요약 내용 등)이 직무 역량과 얼마나 잘 연결되어 어필되는지 분석하고, 프로젝트의 기여도, 기술 스택, 결과물을 더 효과적으로 제시하기 위한 구체적인 개선 방안을 제시해주세요.
...
- 전체적인 피드백은 'overall_feedback'에 담고, 'individual_feedbacks'는 비어있는 객체 {{}}로 반환하거나, 특정 프로젝트별/섹션별 핵심 피드백이 있다면 해당 키로 넣어주세요.
""")
